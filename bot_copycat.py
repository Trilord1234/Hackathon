...
class BotCopycat:
    ...
    def boucle(self):
        print("[Copycat] En attente du début de la partie...")
        while True:
            msg = self._readline()
            if not msg:
                continue
            ...
            if msg.startswith("DEBUT_TOUR"):
                ...
                # 2. Riposte si ennemi trop proche (≤ 2 cases)
                for l, c in mes_unites:
                    for el, ec in ennemis:
                        if actions <= 0:
                            break
                        if distance(l, c, el, ec) <= 2:
                            print(f"  [Copycat] Riposte contre ennemi en ({el},{ec})")
                ...
                # 3. Expansion pacifique loin des ennemis
                cases_libres = [
                    (l, c)
                    for (l, c), d in self.plateau.items()
                    if d['element'] == 'X'
                    and alertes_vers[d['secteur']] != "DANGER"
                ]
                cases_libres.sort(
